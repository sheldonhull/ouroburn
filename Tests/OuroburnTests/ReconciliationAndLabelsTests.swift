...
struct ReconciliationAndLabelsTests {
    ...
    func sessionBucketCarriesLatestCwd() throws {
        let aggregator = Aggregator(pricing: [:])
        let early = try UsageEntry(
            timestamp: #require(ISO8601.parse("2026-05-06T10:00:00.000Z")),
        ...
        let late = try UsageEntry(
            timestamp: #require(ISO8601.parse("2026-05-06T12:00:00.000Z")),
            model: "claude-sonnet-4-20250514",
            inputTokens: 10, outputTokens: 10, cacheCreationTokens: 0, cacheReadTokens: 0,
